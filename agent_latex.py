...
# --- LLM call ---
def llm_chat(system_msg: str, user_msg: str) -> str:
    """
    Dual-path client:
    ...
    """
    use_openai = ("api.openai.com" in OPENAI_BASE_URL) or OPENAI_BASE_URL.endswith("/v1")
...
def compile_pdf(tex_path: str, engine: str = "pdflatex") -> None:
    ...
    def run(cmd):
        subprocess.run(cmd, cwd=tex_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, check=True)
    try:
        run([engine, "-interaction=nonstopmode", fname])
        run([engine, "-interaction=nonstopmode", fname])
    except Exception as e:
        if engine == "pdflatex":
            print("pdflatex failed or not found; trying xelatex...", file=sys.stderr)
            run(["xelatex", "-interaction=nonstopmode", fname])
            run(["xelatex", "-interaction=nonstopmode", fname])
        else:
            raise

# --- Main pipeline ---
def run(pdf_path: str, pages_arg: str, out_prefix: str, compile_flag: bool, title: str):
    # 1) Determine pages
    doc = fitz.open(pdf_path)
    max_pages = len(doc)
    ...
    for (pno, txt) in tqdm(page_texts, desc="Translating to LaTeX"):
        ...
        body = llm_chat(SYSTEM_LATEX, user_msg).strip()
...
# --- CLI ---
def main():
    ap = argparse.ArgumentParser(description="Translate selected PDF pages to LaTeX (equations + TikZ).")
    ap.add_argument("--pdf", required=True, help="Input PDF path")
    ap.add_argument("--pages", required=True, help="Pages to process (e.g., '11' or '11-13' or '3,5,9-12')")
    ap.add_argument("--out", default="translated_pages", help="Output folder (created if missing)")
    ap.add_argument("--title", default="Translated Document", help="Title for the compiled PDF")
    ap.add_argument("--compile", dest="compile", action="store_true", help="Compile the LaTeX to PDF")
    ap.add_argument("--no-compile", dest="compile", action="store_false", help="Do not compile (default)")
